...
file.names <- dir('data')
file.names <- file.names[-which(file.names == 'f-CoServ_weather_corrected.csv')]
...
plot_daily_average <- function(A, building.name = '') {
  ...
  return(ggplot(daily.df, aes(x = date, y = forecast * 96, color = factor(biz_day), group = biz_day)) + 
           geom_point() + labs(color = 'Business Day') + 
    geom_line() + xlab('') + 
    scale_color_manual(values = c("red", "black")) +
    ylab("kWh") + scale_x_date(labels = scales::date_format("%b-%Y")) + 
    ggtitle(sprintf('%s Average Daily Energy Consumption', building.name))) 
}

# Iterate through files and create plots
for (file in c("data/f-APS_weather.csv", "data/f-SRP_weather.csv")) {
  building.name <- unlist(strsplit(file, '-|_'))[2]
  df <- suppressMessages(as.data.frame(read_csv(file)))
  suppressMessages(print(plot_daily_average(df, building.name)))
}
```
...
# Create month column
srp$month <- (month(srp$timestamp))
pge1$month <- month(pge1$timestamp)
...
# PGE1 building
ggplot(filter(pge1, biz_day == 1), aes(x = month, y = 96*30*forecast, group = month)) + 
  geom_boxplot() +
  xlab('Month') + ylab('kWh') + 
  ggtitle('PGE1 Monthly Consumption Boxplots') + scale_x_continuous(breaks = seq(1, 12, 1))

# SRP building,
ggplot(filter(srp, biz_day == 1), aes(x = month, y = 96*30*forecast, group = month)) + 
  geom_boxplot() +
  xlab('Month') + ylab('kWh') + 
  ggtitle('SRP Monthly Consumption Boxplots') + scale_x_continuous(breaks = seq(1, 12, 1))
...
suppressMessages((ggplot(d, aes(as.Date(date, origin = '1970-01-01'), 96*value, col = variable)) + 
  ...
  ggtitle('Average Daily Energy Use all Buildings')))
```
...
knitr::kable(summer.corrs, caption = 'Summer Weather Correlations')
knitr::kable(winter.corrs, caption = 'Winter Weather Correlations')
```
...
ggplot(winter.corrs, aes(x = building.name, y= temp)) + 
  geom_bar(stat = "identity", fill = "blue", color = "black") +
  xlab('Building') + ylab('Pearson Correlation') + 
  ggtitle('Winter Correlation of Temp with Energy Use') +
  scale_y_continuous(breaks = seq(-0.6, 0.6, 0.4), limits = c(-0.6, 0.6))
...
ggplot(summer.corrs, aes(x = building.name, y= temp)) + 
  geom_bar(stat = "identity", fill = "blue", color = "black") +
  xlab('Building') + ylab('Pearson Correlation') + 
  ggtitle('Summer Correlation between Temp and Energy Use') +
  scale_y_continuous(breaks = seq(-0.6, 0.8, 0.4), limits = c(-0.6, 0.8))
```
...
ggpairs(data = srp.daily.summer,
        ...
        title = "SRP Summer Correlations")

ggpairs(data = srp.daily.winter,
        columns = c("ghi", "temp", "rh", "pwat", "ws", "forecast"),
        lower = list(continuous = linear_func),
        title = "SRP Winter Correlations")
```
...
print('Summer SRP Correlation Matrix')
...
print('Winter SRP Correlation Matrix')
...
plot_heatmap <- function(A, season = '') {
  ...
  rownames(weather.matrix) <- A$building.name
...
plot_final_heatmap <- function(melted.A, season = '') {
  ...
  finished.heatmap <- heatmap + 
    geom_text(aes(Building, Weather, label = round(corr,2)), color = "black", size = 4) +
  theme(
    axis.title = element_text(size = 16),
    panel.border = element_blank(),
    panel.background = element_blank(),
    axis.ticks = element_blank())  + 
    guides(fill = guide_colorbar(barheight = 12, label.vjust = 1.3)) +
...
relevant.cols <- c('date', 'forecast', 'ghi', 'dif', 'gti', 'temp', 'rh', 'pwat', 'ws')
...
summer.linear.model <- lm(96 * forecast~temp, data = srp[which(month(srp$date) %in% summer), ])
winter.linear.model <- lm(96 * forecast~temp, data = srp[which(month(srp$date) %in% winter), ])
...
# Plot the daily average energy consumption and temperature
suppressWarnings(print(ggplot(srp.daily.summer, aes(temp, 96 * forecast)) + 
  geom_jitter(size = 2.5) + 
...
the trends of temperature and ghi versus forecast energy. I will continue to work
with the SRP building and segment by season. 

```{r, fig.height = 4 }
srp.daily.summer$day <- format(srp.daily.summer$date, format = "%m-%d")
srp.daily.summer <- srp.daily.summer %>% 
...
ggplot(data = srp.daily.summer) + 
  ...
  geom_point(aes(x = as.Date(day, format = "%m-%d"), y = temp * 2 * 96), size = 2) + 
  geom_line(aes(x = as.Date(day, format = "%m-%d"), y = 96 * forecast, color = 'energy'), size = 1.5) + 
  geom_line(aes(x = as.Date(day, format = "%m-%d"), y = temp * 2 * 96, color = 'temp'), size = 1.5) + 
  scale_y_continuous(sec.axis = sec_axis(~.*(0.5/96), name = "Avg. Temp (C)")) + 
  labs(color = 'legend') + xlab('') + ylab('kWh') + 
  ggtitle('SRP Average Daily Energy Use and Temp.') + 
...
ggplot(filter(results_long, metric == 'rmse' & building != "CoServ" & building != "SMUD"), aes(season)) + 
  geom_bar(aes(y = value, fill = model), 
           color = "black", width = 0.8, position = 'dodge', stat = 'identity') + 
  facet_wrap(~building) + xlab("season") + ylab("rmse (kWh)") + 
  theme(axis.text.x = element_text(angle = 90)) + 
  ggtitle("Model RMSE Comparison") + scale_fill_manual(values = c("blue", "red", "brown"))

ggplot(filter(results_long, metric == 'r_squared' & building != "CoServ" & building != "SMUD"), aes(season)) + 
  geom_bar(aes(y = value, fill = model), color = "black", width = 0.8, position = 'dodge', stat = 'identity') + 
  facet_wrap(~building) + xlab("season") + ylab("r squared") + 
  theme(axis.text.x = element_text(angle = 90)) + 
  ggtitle("Model Rsquared Comparison") + scale_fill_manual(values = c("blue", "red", "brown"))

ggplot(filter(results_long, metric == 'mape' & building != "CoServ" & building != "SMUD"), aes(season)) + 
  geom_bar(aes(y = value, fill = model), color = "black", width = 0.8, position = 'dodge', stat = 'identity') + 
  facet_wrap(~building) + xlab("season") + ylab("mape (%)") + 
  theme(axis.text.x = element_text(angle = 90)) + 
  ggtitle("Model MAPE Comparison") + scale_fill_manual(values = c("blue", "red", "brown"))
```
...
srp_results <- dplyr::filter(results, building == "SRP")
